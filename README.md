...
외부 의존성은 CDN — Pretendard(폰트), Chart.js(분석 차트), Lucide(아이콘), Leaflet(지도).
...
| **후보지 맵** | 좌표를 아는 후보지를 지도에 핀으로. 핀 색은 상태, 글자는 타입. 목록의 검색·필터가 그대로 적용됩니다 |
...
핀 위치는 미리 넣어 둔 `lat`/`lng` 로 찍습니다. 78곳 중 **45곳만** 지도에 올라갑니다.
나머지 33곳은 주소 칸에 건물명(`원그로브`)이나 지번(`역삼동 825-24`)이 들어 있어
지오코딩이 안 됐습니다. 대략 지역 중심에 찍지 않고 **아예 뺍니다** — 거기 있는 것처럼
읽히면 잘못된 정보가 되기 때문입니다. 몇 곳이 빠졌는지는 지도 아래에 적힙니다.

좌표를 다시 뽑거나 새 후보지에 넣으려면 도로명주소를 `주소` 칸에 넣고 지오코딩을 다시 돌리면 됩니다
(Nominatim 은 도로명주소만 찾습니다. `역삼동 825-24` 같은 지번은 못 찾습니다).
