...
| **후보지 맵** | 전체 후보지를 지도에 핀으로. 핀 색은 상태, 글자는 타입. 목록의 검색·필터가 그대로 적용됩니다 |
...
핀 위치는 미리 넣어 둔 `lat`/`lng` 로 찍습니다. 78곳 중 45곳은 주소로 정확히 찾았고,
나머지 33곳은 주소가 건물명이거나 지번이라 지오코딩이 안 돼서 **지역(구/시) 중심점**에
점선 테두리 핀으로 찍습니다. 겹치지 않게 id 로 정해지는 만큼 흩뿌립니다.
