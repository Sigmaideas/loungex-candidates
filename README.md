...
| 결과 | **상태** — 후보지 · 협의중 · 계약완료 |
| 층수 · 전용 면적 | 층 · 면적(평) |
| 보증금 · 권리금 · 고정 임차료 · 수수료율 · 건물 관리비 | 임대 조건 |
| (시트에 없음) | **투자금** — 보증금·권리금과 별개로 들어가는 투자 비용 (인테리어·시설 등) |
| 계약 가능시기 · 기타 | 진행 |
...
app.js                상태 · 렌더 · 저장 · 지도
config.js             공유 저장소(워커) 주소 · 계약서 링크 목록
...
| **계약** | 계약서 다운로드. 파일은 구글 드라이브에 두고 링크만 겁니다 |
...
`config.js` 의 `LOUNGEX_CONTRACTS` 목록을 그대로 화면에 뿌립니다.

```js
{ name: '임대차 계약서', desc: '점포 임대차 표준계약서', url: '' }
```

`url` 에 구글 드라이브 공유 링크를 넣으면 그 칸이 활성화되고, 비어 있으면
**링크 미등록** 으로 회색 처리됩니다. 칸 자체를 감추지 않는 이유는, 감추면
"그 문서가 없는 건지 링크만 안 걸린 건지" 구분이 안 되기 때문입니다.

파일은 드라이브에 있으므로 **누가 받을 수 있는지는 드라이브 공유 설정이 정합니다.**
이 화면은 링크를 새 탭으로 열어 줄 뿐 권한을 따로 확인하지 않습니다.
